...
function TodoList() {
    ...
    const addTodo = () => {
        ...
        // value 값이 input 으로 들어온 value 값을 text 라는 key에 저장
        // complete = 무언가를 끝내다 라는 영어
        // 할 일 추가 = 끝내야 하는 일
    ...
    const toggleTodo = (index) => {
      ...
      // 완료 상태면 미완료 상태로 변경하고, 미완료 상태면 완료 상태로 변경하곘다
      // newTodos[index].completed = !newTodos[index].completed
    ...
    return (
      <div className="TodoList">
        ...
        {/* 할 일이 추가되면 추가된 목록을 system.out.print 와 같은 map 을 이용해서
            사용자 눈에 추가된 목록을 보여주기
            각 목록은
            <li key={목록리스트번호순}> {목록번호에 맞고, value=input에 작성한 text를 보여줌} </li>
            우리가 li 태그를 클릭할 때마다 할 일을 완료했는지, 미완료인지 눈으로 확인할 수 있게 표기
            textDecoration = 밑줄
            만약에 할 일이 true  면 line-through
            만약에 할 일이 false 면 none 이용해 줄 삭제
            line-through = 중간에 줄 긋기
...
/*
 추후 useEffect 를 사용해서 웹사이트를 새로고침 하더라도 목록이 남아있을 수 있도록 업데이트!!!
