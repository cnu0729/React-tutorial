...
const MovieRating = () => {
    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            //임의의 변수면 res 사용 res = [].data 데이터를 가져오겠다
            //주소값에 key 이름으로 작성된 data에서 movie 라는 키 값 안에 적힌 title과 이미지들을 가져올 것
            //         [ { data{movie{ 를 가져올 것 }}} ]
            set영화들(res.data.data.movies); // {}중괄호들에서 data를 가지고 오곘다.
            //영화 데이터 모두 가지고 오면 로딩중을 false로 변경하기
            set로딩(false);
        })
        .catch(err => {
            alert(err + "발생했습니다.");
    ...
    영화 데이터에서 이미지가 있지 때문에 데이터가 느리게 가지고 옴
    사용자에게 화면이 보여지기 전에 로딩중이라는 표시를 보여줌으로
    사용자는 조금 더 기다릴 수 있는 시간을 갖게됨
    처음엔 로딩 true로 설정해서 임시화면을 띄워놓고
