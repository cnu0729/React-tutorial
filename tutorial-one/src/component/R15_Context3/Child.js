...
const Child = () => {
    ...
    const 사용자추가하기 = () => {
        //상태변수값을 이용해서 user 객체 생성
        //const 유저 = {"부모에전달할변수명" : 부모에전달할값}; 
        const 유저 = {"inputName" : inputName, "inputAge" : inputAge}; // 이름 맞추는게 보기 편함

        // userList 복사 = ...userList(기존 유저 리스트) + 유저 데이터 추가
        // 일상 대화에서 .. 을 쓰면 많은 내용이 함축적으로 담겨있는 것처럼
        // ...기존유저리스트에는 모든 윶의리스트를 담아진 값을 복사해놓겠다.
        // ...변수명 = 변수명에 담긴 모든 값을 복사해놓겠다.
        const 새로운유저 = [...userList, 유저]; 
        // 기존 내용을 복제하고 복제한 리스트에 새로운 유저를 추가해서 덮어쓰기
    ...
    return (
        <div>
            ...
            <input type="text" id="inputId" onChange={(e) => {setInputName(e.target.value)}} value={inputName} /><br />
            ...
            {/* 
                ...
                e(움직임이 발생).target(움직임이 발생한 목표).value(값이 무엇인지 가져옴)
