...
const Signup = () => {
    ...
    const 아이디중복검사 = (eventId) => {
        ...
        // 4글자 미만이면 중복 검사 X
        // 입력받은 아이디 공백제거하고 공백제거한 총 길이가 4보다 적다면
        // trim() 양쪾 옆에 있는 공백제거 length = 길이
        if(eventId.trim().length < 4){ 
            setIdValidation(false);
            return;
        }
        // db에 중복되는 아이디가 있는지 비동기로 아이디 중복검사 수행
        // axios 나 fetch를 사용할 수 있음
        fetch("/idCheck?id=" + eventId) // springboot controller와 연결할 MAPPING URL
        .then(response => response.text())
        .then(result => {
            // 중복이 아닐 때 true, 중복이면 false
            // if(Number(result) === 0 나중에 SPRING BOOT 에서
            // 아이디가 중복되지 않았으면 0을 전달 아이딕다 중복이면 0 이외 값을 전달
            // 결과 전달을 0 사용하지 않고 "중복아님" 글자 전달 IF(){}
            if(Number(result) === 0){
    ...
    const 회원가입버튼 = () => {
        ...
        */
        ...
        fetch("/Signup", {
            ...
            // application/json : application = 코딩하는폴더 자체를 의미 폴더1개 = 어플리케이션 1개
            // json = 사용자나 개발자가 작성한 여러값을 key-value형태로 데이터를 1개이상 주고 받을 수 있는 형태
    ...
    return (
        <div className="signup-container">
            {/* label 태그 안에 for 값을 작성하고 싶지 않다면 
                <label for="input 에 작성된 id값과 일치하는 값을 작성"></label>
                <label><input /></label> 
